...
export default function TestRunPage() {
  ...
  const handleDownloadLog = () => {
    if (!testRun || !testRun.steps) return
    
    const logs = testRun.steps.map(step => ({
    ...
    const logContent = [
      ...
      `Total Steps: ${testRun.steps.length}`,
      `Successful: ${testRun.steps.filter(s => s.success).length}`,
      `Failed: ${testRun.steps.filter(s => !s.success).length}`,
      '',
      '=== Test Steps ===',
      '',
      ...logs.map(log => 
        `[${new Date(log.timestamp).toLocaleTimeString()}] Step ${log.stepNumber}: ${log.action}${log.target ? ` → ${log.target}` : ''}${log.value ? ` (${log.value})` : ''} ${log.success ? '✓' : '✗'}${log.error ? `\n  Error: ${log.error}` : ''}`
      )
    ].join('\n')
