...
export class PlaywrightRunner {
  ...
  async reserveSession(profile: TestProfile): Promise<RunnerSession> {
    ...
    console.log('Playwright: Browser config:', {
  ...
  async captureElementScreenshot(sessionId: string, selector: string): Promise<string> {
    ...
    console.log('Playwright: Capturing element screenshot for:', selector)
    ...
    try {
      ...
      console.log('Playwright: Element screenshot captured successfully')
      return screenshot as string
    } catch (error: any) {
      console.error('Playwright: Failed to capture element screenshot:', error.message)
  ...
  async executeAction(sessionId: string, action: LLMAction): Promise<ActionExecutionResult | void> {
    ...
    try {
      ...
      switch (action.action) {
        ...
        case 'check':
          // Check a checkbox (mark as checked)
          if (!action.selector) {
            throw new Error('Selector required for check action')
          }
          
          console.log('Playwright: Checking checkbox:', action.selector)
          ...
          try {
            ...
            if (!isChecked) {
              ...
              console.log('Playwright: Checkbox checked successfully')
            } else {
              console.log('Playwright: Checkbox already checked, skipping')
            }
          } catch (error: any) {
            const { formatErrorForStep } = await import('../utils/errorFormatter')
            const formattedError = formatErrorForStep(error, { action: action.action, selector: action.selector })
        ...
        case 'uncheck':
          // Uncheck a checkbox (mark as unchecked)
          if (!action.selector) {
            throw new Error('Selector required for uncheck action')
          }
          
          console.log('Playwright: Unchecking checkbox:', action.selector)
          ...
          try {
            ...
            if (isChecked) {
              ...
              console.log('Playwright: Checkbox already unchecked, skipping')
            }
          } catch (error: any) {
            const { formatErrorForStep } = await import('../utils/errorFormatter')
            const formattedError = formatErrorForStep(error, { action: action.action, selector: action.selector })
        ...
        case 'select':
          ...
          console.log('Playwright: Selecting option:', action.value, 'from:', action.selector)
          ...
          try {
            ...
            try {
              ...
              console.log('Playwright: Option selected successfully')
            } catch (selectError) {
              // If direct selection fails, try by label
              try {
                await locator.selectOption({ label: action.value }, { timeout: 10000 })
                console.log('Playwright: Option selected by label successfully')
              } catch (labelError) {
                ...
                if (!isNaN(Number(action.value))) {
                  ...
                  console.log('Playwright: Option selected by index successfully')
                } else {
                  throw selectError
                }
              }
            ...
            const { formatErrorForStep } = await import('../utils/errorFormatter')
            const formattedError = formatErrorForStep(error, { action: action.action, selector: action.selector })
            throw new Error(`Failed to select option "${action.value}" in ${action.selector}: ${formattedError}`)
          }
          break
          
        case 'goBack':
          // Navigate back in browser history
          console.log('Playwright: Navigating back in browser history')
          
          try {
            await page.goBack({ waitUntil: 'networkidle', timeout: 30000 })
            await page.waitForTimeout(500) // Wait for page to stabilize
            console.log('Playwright: Navigated back successfully')
          } catch (error: any) {
            const { formatErrorForStep } = await import('../utils/errorFormatter')
            const formattedError = formatErrorForStep(error, { action: action.action })
        ...
        case 'goForward':
          ...
          console.log('Playwright: Navigating forward in browser history')
          
          try {
            await page.goForward({ waitUntil: 'networkidle', timeout: 30000 })
            await page.waitForTimeout(500) // Wait for page to stabilize
            console.log('Playwright: Navigated forward successfully')
          } catch (error: any) {
            const { formatErrorForStep } = await import('../utils/errorFormatter')
            const formattedError = formatErrorForStep(error, { action: action.action })
        ...
        case 'submit':
          // Submit a form (press Enter on the element or submit the form)
          if (!action.selector) {
            throw new Error('Selector required for submit action')
          }
          
          console.log('Playwright: Submitting form via:', action.selector)
          ...
          try {
            ...
            try {
              ...
              console.log('Playwright: Form submitted via Enter key')
            } catch (enterError) {
              // If Enter doesn't work, try clicking if it's a button
              try {
                await locator.click({ timeout: 10000 })
                console.log('Playwright: Form submitted via click')
              } catch (clickError) {
                ...
                console.log('Playwright: Form submitted via form.submit()')
              }
            }
