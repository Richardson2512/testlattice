...
export class TestProcessor {
  ...
  private async captureDiagnosisSnapshot(params: {
    ...
    const flows = semanticAnalysis.testableComponents.map((comp, idx) => {
        ...
        try {
            ...
            if (isMonkeyMode) {
              ...
              if (history.length > 0) {
                ...
                if (lastAction.selector) {
                  ...
                  try {
                    const selectorExists = await session.page.evaluate((sel) => {
                      try {
          ...
          if (
            ...
            this.visionValidator.shouldUseVision({
              ...
              criticalError: previousActionFailed || false,
            })
          ) {
            try {
