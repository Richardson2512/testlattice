...
export class TestProcessor {
  ...
  private async captureDiagnosisSnapshot(params: {
    ...
    // For full-page analysis, we should use the full DOM which already contains all elements
    // The analyzeScreenshot method already extracts all elements from the DOM, so we're good
    // But we can enhance it by analyzing multiple screenshots if needed
    ...
    try {
      await Promise.all([
        ...
        this.comprehensiveTesting.checkResponsive(page), // Responsive/mobile testing
        this.comprehensiveTesting.checkErrorHandling(page, params.build.url || ''), // NEW: Error handling testing
      ])
    ...
    // Analyze page testability with the full context
    ...
    if (params.options?.criticalPath?.enabled) {
      ...
      try {
        ...
        console.log(`[${params.runId}] Critical path flow: ${criticalPathResult.completedSteps}/${criticalPathResult.totalSteps} steps completed`)
      } catch (error: any) {
        console.error(`[${params.runId}] Critical path flow failed:`, error.message)
      }
    }
   ...
   */
  private async checkForManualAction(runId: string): Promise<LLMAction | null> {
    try {
      const fetch = (await import('node-fetch')).default
  ...
  private async performDiagnosisCrawlWithProgress(options: {
    ...
    const linkElements = baseContext.elements.filter(
      (el) =>
        ...
        el.text &&
        !el.isHidden &&
        !/^(login|sign.?in|log.?out|sign.?out|register|forgot|reset)/i.test(el.text)
    )
...
`
      ...
      while (stepNumber < maxSteps && Date.now() - startTime < maxDuration) {
        ...
        if (paused) {
          ...
          const manualAction = await this.checkForManualAction(runId)
          if (manualAction) {
        ...
        try {
          ...
          if (originalSelectorBeforeHealing && selectorHealingMap.has(originalSelectorBeforeHealing)) {
            ...
            console.log(`[${runId}] [${browserType.toUpperCase()}] Step ${stepNumber}: Using learned selector override ${originalSelectorBeforeHealing} → ${healedSelector}`)
          }
            ...
            try {
              ...
              if (baselineStep?.screenshotUrl) {
                ...
                if (!isAcceptable && diffResult.diffImageBuffer) {
                  ...
                  console.log(`[${runId}] [${browserType.toUpperCase()}] Visual regression detected: ${diffResult.diffPercentage.toFixed(2)}% difference`)
                } else {
                  visualDiffResult = {
                    hasDifference: false,
                    diffPercentage: diffResult.diffPercentage,
  ...
  async process(jobData: JobData): Promise<ProcessResult> {
    ...
    if (this.visionValidator) {
      this.comprehensiveTesting.setVisionValidator(this.visionValidator)
    }
