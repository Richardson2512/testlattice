...
export class WebRTCStreamer extends EventEmitter {
  ...
  private async startFrameCapture(page: Page): Promise<void> {
    try {
      ...
      // Updated to 30fps (33ms interval) for smooth experience
  ...
  private async startScreenshotBasedStream(page: Page): Promise<void> {
    try {
      ...
      }, 33) // 30fps for smooth experience (1000ms / 30 ≈ 33ms)

      console.log('[WebRTC] Screenshot-based streaming started (30fps)')
    } catch (error: any) {
