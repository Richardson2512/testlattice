...
export class UnifiedBrainService {
   ...
   */
  private shouldFallbackTo14B(response: any, context?: any): boolean {
   ...
   */
  private getFallbackReason(response: any, context?: any): string {
...
- assert: Verify element state (value format: "type:expected")
- complete: Mark test as complete`
  ...
  private buildActionSystemPrompt(goal: string, trackingInfo?: any): string {
    ...
    parts.push('3. Always prefer interactive actions (click, type, check, uncheck, select, submit) over passive actions')
    ...
    parts.push('7. For checkboxes/radio buttons, use "check" or "uncheck" actions, NOT "click"')
    parts.push('8. For dropdowns (<select>), use "select" action with option value/label')
    parts.push('9. For form submission, use "submit" action instead of clicking submit button')
    parts.push('10. Return valid JSON with action, selector, description, and confidence (0.0-1.0)')
...
Return JSON:
{
  ...
  getMetrics() {
    return {
      ...this.metrics,
      fallbackRate: this.metrics.totalCalls > 0 ? this.metrics.fallbackUsed / this.metrics.totalCalls : 0,
      fallbackSuccessRate:
        this.metrics.fallbackUsed > 0 ? this.metrics.fallbackSuccess / this.metrics.fallbackUsed : 0,
    }
  }
}
