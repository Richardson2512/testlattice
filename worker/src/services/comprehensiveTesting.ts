...
import { ResponsiveTestingService } from './responsiveTesting'
import { ErrorHandlingTestingService } from './errorHandlingTesting'
import { CriticalPathTestingService, FlowExecutionResult } from './criticalPathTesting'
...
export class ComprehensiveTestingService {
    ...
    if (viewportInfo) {
      const buttonsOutsideViewport = await page.evaluate((vpWidth, vpHeight) => {
        ...
        buttons.forEach((btn) => {
          ...
          else if (rect.top < 0 && rect.bottom > 0) position = 'partially above viewport'
          else if (rect.left < 0 && rect.right > 0) position = 'partially left of viewport'
          else if (rect.bottom > vpHeight && rect.top < vpHeight) position = 'partially below viewport'
          else if (rect.right > vpWidth && rect.left < vpWidth) position = 'partially right of viewport'
          ...
          if (position) {
            ...
            } else selector = btn.tagName.toLowerCase()
      ...
      buttonsOutsideViewport.forEach((item) => {
        issues.push({
          type: 'missing-element',
          element: item.element,
          selector: item.selector,
          description: `Interactive element ${item.position}: ${item.selector} (top: ${item.rect.top.toFixed(0)}px, left: ${item.rect.left.toFixed(0)}px)`,
          severity: item.position.includes('partially') ? 'medium' : 'high',
          recommendation: `Ensure ${item.selector} is fully visible within the viewport or add scroll-to-view behavior`
        })
      })
    }
  ...
  /**
   * Check for horizontal scrolling on desktop viewports
   * Horizontal scrolling is generally undesirable on desktop layouts
   */
  async checkHorizontalScroll(page: Page): Promise<VisualIssue[]> {
    ...
    // Only check for horizontal scroll on desktop viewports (width >= 1024)
    if (viewport.width < 1024) {
      console.log('Skipping horizontal scroll check (mobile/tablet viewport)')
      return issues
    }
    ...
    const scrollData = await page.evaluate(() => {
      ...
      allElements.forEach((el) => {
        ...
        if (rect.right > windowWidth && style.display !== 'none' && style.visibility !== 'hidden') {
          let selector = ''
          if (el.id) selector = `#${el.id}`
          else if (el.className) {
            const className = typeof el.className === 'string' 
          ...
          overflowingElements.push({
            ...
            element: el.tagName.toLowerCase(),
            width: rect.width,
            computedWidth: rect.right - rect.left
          })
        }
      })
    ...
    if (scrollData.hasHorizontalScroll) {
      issues.push({
        type: 'layout-shift',
        description: `Horizontal scrolling detected on desktop: page is ${scrollData.overflow}px wider than viewport (${scrollData.windowWidth}px)`,
        severity: 'high',
        expectedValue: `Page width should be ≤ ${scrollData.windowWidth}px`,
        actualValue: `${Math.max(scrollData.bodyWidth, scrollData.htmlWidth)}px`,
        recommendation: `Fix responsive layout: check for fixed widths, large images, or elements with overflow. Culprits: ${scrollData.overflowingElements.slice(0, 3).map(e => e.selector).join(', ')}`
      })
      
      // Log specific overflowing elements
      scrollData.overflowingElements.slice(0, 5).forEach((elem) => {
        issues.push({
          type: 'element-overlap',
          element: elem.element,
          selector: elem.selector,
          description: `Element extends beyond viewport: ${elem.selector} (width: ${elem.width.toFixed(0)}px)`,
          severity: 'medium',
          recommendation: `Adjust width or add responsive styles to ${elem.selector}`
        })
      })
    }
  ...
  /**
   * Check basic layout structure (header, footer, main content visibility)
   * Ensures page has proper semantic structure and all key areas are visible
   */
  async checkLayoutStructure(page: Page): Promise<VisualIssue[]> {
    ...
    const structure = await page.evaluate(() => {
      ...
      const hasContent = (el: Element | null): boolean => {
        ...
        return text.length > 0 || children > 0
      }
      
      // Check for blank screen (no visible content)
      const bodyRect = document.body.getBoundingClientRect()
      const bodyStyle = window.getComputedStyle(document.body)
      const isBodyVisible = bodyStyle.display !== 'none' && bodyRect.height > 0
      ...
      const hasVisibleContent = visibleElements.length > 10 // Arbitrary threshold
    ...
    if (!structure.isBodyVisible || !structure.hasVisibleContent) {
      issues.push({
        type: 'missing-element',
        description: `Blank screen detected: page appears empty or not fully loaded (${structure.visibleElementCount} visible elements)`,
        severity: 'critical',
    ...
    if (!structure.hasHeader) {
      issues.push({
        type: 'missing-element',
        element: 'header',
        description: 'No header element found on page',
        severity: 'medium',
        recommendation: 'Add a <header> element or element with role="banner" for better semantic structure'
      })
      ...
      issues.push({
        type: 'missing-element',
        element: 'header',
        description: 'Header element is empty (no content)',
        severity: 'medium',
    ...
    if (!structure.hasMain) {
      issues.push({
        type: 'missing-element',
        element: 'main',
        description: 'No main content element found on page',
        severity: 'high',
        recommendation: 'Add a <main> element or element with role="main" for primary content'
      })
    ...
    if (!structure.hasFooter) {
      issues.push({
        type: 'missing-element',
        element: 'footer',
        description: 'No footer element found on page',
        severity: 'low',
        recommendation: 'Add a <footer> element or element with role="contentinfo" for better semantic structure'
      })
   ...
   * Tests touch targets, text size, horizontal scroll, breakpoints, etc.
   */
  async checkResponsive(page: Page): Promise<VisualIssue[]> {
    ...
    try {
      ...
      console.log(`Found ${visualIssues.length} responsive/mobile issues`)
      
      // Add to visual issues collection
      this.visualIssues.push(...visualIssues)
      
      return visualIssues
    } catch (error) {
      console.error('Error in responsive testing:', error)
      return []
    }
  }
   ...
   * Tests user-facing errors, error recovery, console errors, CORS, 404s, etc.
   */
  async checkErrorHandling(page: Page, baseUrl: string): Promise<VisualIssue[]> {
    ...
    try {
      ...
      // Convert to VisualIssue format for reporting
      const visualIssues: VisualIssue[] = errorIssues.map(issue => ({
        type: issue.type === 'stack-trace-exposed' ? 'missing-element' : 
              issue.type === 'cors-error' ? 'missing-element' :
              issue.type === 'asset-404' ? 'missing-element' :
              'missing-element',
        description: issue.description,
        severity: issue.severity === 'critical' || issue.severity === 'high' ? 'high' : 
      ...
      console.log(`Found ${visualIssues.length} error handling issues`)
      ...
      console.error('Error in error handling testing:', error)
      return []
    }
  }
   ...
   * Execute end-to-end user flows with state persistence validation
   */
  async runCriticalPathFlow(
    ...
    console.log(`Running critical path flow: ${flowName}`)
    
    const result = await this.criticalPathTesting.executeFlow(page, flowName, baseUrl)
    
    console.log(`Flow '${result.flowName}' completed: ${result.completedSteps}/${result.totalSteps} steps (${result.duration}ms)`)
    
    return result
  }

  /**
   * Get available critical path flows
   */
  ...
  reset(): void {
    ...
    this.wcagScore = null
    this.errorHandlingTesting.reset()
    this.criticalPathTesting.reset()
  }
}
