  ...
  fastify.post<{ 
    ...
    try {
      ...
      const manualAction = {
        ...
        value,
        description,
        godMode: request.body.godMode,  // NEW: Include God Mode data
        timestamp: new Date().toISOString(),
      }
      ...
      ws.broadcast(runId, {
        type: request.body.godMode ? 'god_mode_click' : 'manual_action_injected',
        action: manualAction,
        timestamp: new Date().toISOString(),
      })

      fastify.log.info(`${request.body.godMode ? 'God Mode' : 'Manual'} action injected for test run ${runId}:`, action, selector)
