...
export interface PricingTierLimits {
  ...
  maxBehaviorTests: number  // Behavior add-on (Indie/Pro only)
  ...
  behaviorAddonAvailable: boolean
}
...
export const PRICING_TIERS: Record<PricingTier, PricingTierInfo> = {
  free: {
    ...
    limits: {
      ...
      maxVisualTests: 1,
      maxBehaviorTests: 0,  // Not available on Free
      browsers: ['chromium'],
...
// Behavior Test Add-on (Credits-based purchase for Indie/Pro)
export interface BehaviorTestAddOn {
  id: string
  name: string
  credits: number  // Number of behavior tests per purchase
  price: number
  priceLabel: string
  description: string
...
export const BEHAVIOR_TEST_ADDON: BehaviorTestAddOn = {
  id: 'behavior-credits',
  name: 'Behavior Analysis Credits',
  credits: 20,
  price: 20,
  priceLabel: '$20',
  description: '20 AI-powered chatbot/agent behavior tests. Can be purchased multiple times.',
  polarProductId: 'fd537924-bd3a-43e1-8668-4bd8334fa60b',
}
