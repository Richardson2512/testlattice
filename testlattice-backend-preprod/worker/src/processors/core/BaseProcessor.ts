...
import { ExertionCalculator } from '../../services/ExertionCalculator'
...
export abstract class BaseProcessor {
    ...
    protected exertionCalculator: ExertionCalculator
    ...
    constructor(
        ...
        // Initialize Composition Helpers
        this.statusReporter = new StatusReporter(this.runId, {
            supabase: deps.supabase,
            stateManager: deps.stateManager
        })
        this.exertionCalculator = new ExertionCalculator()
    }
        ...
        return withTraceAsync(
            ...
            async () => {
                ...
                try {
                    ...
                    // Post-processing: Calculate Exertion/Confidence
                    const exertion = this.exertionCalculator.calculate(
                        this.steps,
                        this.startTime,
                        Date.now(),
                    ...
                    // Post-processing: Attach Vibe Engine Metadata
                    const vibeMeta = {
                        engineVersion: "1.0.0",
                        model: TEST_MODE_CONFIGS[this.testMode].aiModel,
                        heuristicVersion: "2026.01.alpha",
                        provider: 'openai'
                    } as const
    ...
    protected async setup(): Promise<void> {
        ...
        await this.statusReporter.updateStatus('running')
    ...
    protected async handleError(error: any): Promise<ProcessResult> {
        ...
        // Update status
        await this.statusReporter.updateStatus('failed', userMessage)
