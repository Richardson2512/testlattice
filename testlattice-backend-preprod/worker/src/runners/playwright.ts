...
import { validateUrlOrThrow, isUrlSafe } from '../utils/urlValidator'
import { networkGuardian } from '../security/NetworkGuardian'
...
export class PlaywrightRunner {
   ...
   */
  async reserveSession(profile: TestProfile, options?: { skipVideo?: boolean }): Promise<RunnerSession> {
    ...
    // Only record video if not explicitly skipped
    ...
    await page.route('**/*', async (route) => {
      ...
      try {
        ...
        const { safe, reason } = await networkGuardian.validateUrl(requestUrl)
  ...
  async scrollToPosition(sessionId: string, y: number): Promise<void> {
    ...
    try {
      ...
      // No need for arbitrary timeout, evaluate is sufficient
    } catch (error: any) {
